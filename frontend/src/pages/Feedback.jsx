        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="pl-10 w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-[#E4141C]"
              value={searchTerm}
              onChange={(e) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center">
            <div className="bg-[#242C54] text-white p-3 rounded-full mr-4">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          <div className="flex items-center">
            <div className="bg-[#E4141C] text-white p-3 rounded-full mr-4">
              ...
              <li key={item.id} className="p-4 hover:bg-gray-50">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start space-x-4">
                    <div className="bg-[#242C54] text-white p-3 rounded-full">
                      <FiUser />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-medium text-[#242C54] truncate">
                          {item.passenger}
                        </p>
                        <div className="ml-2">
                      ...
                      <p className="text-sm text-gray-500 mb-1">{item.station} • {item.date}</p>
                      {renderStars(item.rating)}
                      <p className="mt-2 text-gray-700">{item.comments}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button className={`px-3 py-1 rounded text-sm ${item.followUp ? 'bg-[#E4141C] text-white' : 'bg-gray-100 text-gray-700'}`}>
      ...
      {filteredFeedback.length > feedbackPerPage && (
        <div className="flex items-center justify-between">
          <button
            ...
            className={`flex items-center px-4 py-2 rounded ${currentPage === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-[#242C54] hover:bg-gray-100'}`}
          >
          ...
          <div className="flex space-x-1">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(number => (
              <button
                ...
                className={`w-10 h-10 rounded-full ${currentPage === number ? 'bg-[#E4141C] text-white' : 'text-[#242C54] hover:bg-gray-100'}`}
              >
                {number}
              </button>
            ))}
          ...
          <button
            ...
            className={`flex items-center px-4 py-2 rounded ${currentPage === totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-[#242C54] hover:bg-gray-100'}`}
          >
